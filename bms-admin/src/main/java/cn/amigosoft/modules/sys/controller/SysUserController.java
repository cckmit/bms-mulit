...
public class SysUserController {
    ...
    public Result<Object> save(@RequestBody SysUserDTO dto) {
        ...
        object.setMsg("操作成功");
        String cardAccount = dto.getCardAccount();
        if (StringUtils.isNotEmpty(cardAccount)) {
            QueryWrapper<SysUserEntity> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("card_account", cardAccount);
            SysUserEntity data = sysUserService.selectOne(queryWrapper);
            if (data != null) {
                return object.error("该外部账号已被绑定");
            }
        }
        QueryWrapper<SysUserEntity> wrapper = new QueryWrapper<>();
        wrapper.eq("mobile", dto.getMobile());
        if (sysUserService.selectOne(wrapper) != null) {
            return object.error("该手机号已被绑定");
        }
    ...
    public Result<Object> update(@RequestBody SysUserDTO dto) {
        ...
        QueryWrapper<SysUserEntity> wrapper = new QueryWrapper<>();
        wrapper.eq("mobile", dto.getMobile());
        SysUserEntity queryMobileData = sysUserService.selectOne(wrapper);
        if (queryMobileData != null && (!queryMobileData.getId().equals(dto.getId()))) {
            return object.error("该手机号已被绑定");
        }
