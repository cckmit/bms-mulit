...
public class SysDeptController {
    ...
    @PutMapping("updateManger")
    @ApiOperation("修改委员")
    @LogOperation("修改委员")
//	@RequiresPermissions("sys:dept:update")
    public Result updateManger(@RequestBody SysDeptDTO dto) {
        //效验数据
//		ValidatorUtils.validateEntity(dto, UpdateGroup.class, DefaultGroup.class);
//
        sysDeptService.updateManger(dto);
    ...
    @ApiOperation("列表查询（登录用户的顶级租户下的所有部门），用于选择人员")
    public Result<List<SysDeptDTO>> superList() {
        List<SysDeptDTO> list = sysDeptService.superList(new HashMap<>(1));
