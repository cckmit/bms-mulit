        ...
        for (String keyName : keyNames) {
            ...
            if (keyName.contains("ew.paramNameValuePairs.") && sql.toLowerCase().contains(" like ?")) {
                ...
                if (keyName.contains(".")) {
                    ...
                    Map<String, Object> map = JSONObject.parseObject(JSON.toJSONString(parameter.get(split[0])));
                    parameter.put(keyName, EscapeUtil.escapeChar(map.get(split[1]).toString()));
                } else {
                    Object a = parameter.get(keyName);
                    if (a instanceof String && (a.toString().contains("_") || a.toString().contains("\\") || a.toString()
                            .contains("%"))) {
